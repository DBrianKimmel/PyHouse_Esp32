...
void mqtt_task(void *pvParameters) {
	Client_t *l_client = (Client_t *) pvParameters;
	ESP_LOGI(TAG, "214 Task - Begin.  l_client:%p", l_client);
	while (1) {
		...
		if (!mqtt_connect(l_client)) {
			ESP_LOGE(TAG, "218 Task - Connect Failed");
			continue;
		}
		ESP_LOGI(TAG, "221 Task - Connected to MQTT broker, create sending thread before call connected callback");
...
//	p_client->State.connect_info = &p_client->connect_info;
	...
	ESP_LOGI(TAG, "311 Start - Creating another task - send_rb:%p; Packet:%p", p_client->send_rb, p_client->Packet);
...
//	vTaskDelay(10000);
	...
	ESP_LOGI(TAG, "342 Connect - Sending MQTT CONNECT message, MsgType: %d, ClientId: %04X", p_client->State->pending_msg_type, p_client->State->pending_msg_id);

	ESP_LOGI(TAG, "344 Connect - Write Socket");
	int write_len = write(p_client->Broker->Socket, p_client->State->outbound_message->PayloadData, p_client->State->outbound_message->PayloadLength);
	ESP_LOGI(TAG, "346 Connect - Write Len: %d - %d ", write_len, p_client->State->outbound_message->PayloadLength)

	ESP_LOGI(TAG, "348 Connect - Reading MQTT CONNECT response message");
	int read_len = read(p_client->Broker->Socket, p_client->Buffers->in_buffer, CONFIG_MQTT_BUFFER_SIZE_BYTE);
	ESP_LOGI(TAG, "350 Connect - ReadLen: %d - %d", read_len, sizeof(p_client->Buffers->in_buffer));

	tv.tv_sec = 0; /* No timeout */
	setsockopt(p_client->Broker->Socket, SOL_SOCKET, SO_RCVTIMEO, (char *)&tv, sizeof(struct timeval));

	if (read_len < 0) {
		ESP_LOGE(TAG, "356 Connect - Error network response");
	...
	switch (connect_rsp_code) {
		case CONNECTION_ACCEPTED:
			ESP_LOGI(TAG, "366 Connect - Connected");
			return ESP_ERR_MQTT_OK;
		...
		case CONNECTION_REFUSE_NOT_AUTHORIZED:
			ESP_LOGW(TAG, "373 Connect - Connection refused, reason code: %d", connect_rsp_code);
			return ESP_ERR_MQTT_FAIL;
		default:
			ESP_LOGW(TAG, "376 Connect - Connection refused, Unknown reason");
			return ESP_ERR_MQTT_FAIL;
	}
	return ESP_ERR_MQTT_OK;
}
